...
class Dataset(udata.Dataset):
    def __init__(self, data_path, color_mode=False, sigma=25, oracle_mode=0, past_frames=7, future_frames=7, search_window_width=41, nn_patch_width=41, pass_nn_value=False, patch_width=44, patch_stride=5):
        ...
        self.sigma = sigma
        # Here is in the root train_set, each file in categories is a dir
        # We can assume that, for each video, we have a directory, then in this directory are still directories, each stands for a burst
        categories = os.scandir(data_path)
        categories = [c for c in categories if not c.name.startswith('.') and c.is_dir()]

        video_paths_dict = {}
        video_paths_list = []
        for c in categories: # Here is for each video, each video in 'video_paths_dict' has an entry
            list_for_category = []
    ...
    def prepare_epoch(self):
        ...
        for c in self.categories:
            ...
            # for p in paths[:3]:
            for p in paths[:1]: # Modified by HYZ
